...
//power
 ...
 // damageSoundTime(); bugou o jogo
  ...
  let gameStarted = false; // Estado inicial do jogo
...
function showStartScreen() {
  ...
  // Desenhar o logo do jogo
  const logoX = canvas.width / 2 - 100; // Centralizado horizontalmente
  const logoY = 50; // Posição vertical do logo
  const logoWidth = 200;
  ...
  // Configurar o botão "Jogar"
  const buttonX = canvas.width / 2 - 70; // Posição X
  const buttonY = 200; // Posição Y
  const buttonWidth = 150;
  ...
  context.fillStyle = "white";
  context.font = "20px Arial";
  context.fillText("Play", buttonX + 50, buttonY + 27);
...
function startGame() {
  ...
  // Limpar o canvas para desenhar o jogo
  context.clearRect(0, 0, canvas.width, canvas.height);

  // Exemplo de implementação do nível 1
  function gameLoop() {
